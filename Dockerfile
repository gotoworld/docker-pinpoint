...
ADD http://mirror.apache-kr.org/maven/maven-3/3.5.0/binaries/apache-maven-3.5.0-bin.tar.gz ./
ADD http://www.apache.org/dist/maven/maven-3/3.5.0/binaries/apache-maven-3.5.0-bin.tar.gz.md5 ./
RUN [ $(md5sum apache-maven-3.5.0-bin.tar.gz | grep --only-matching -m 1 '^[0-9a-f]*') = $(cat apache-maven-3.5.0-bin.tar.gz.md5) ]
RUN tar -xf apache-maven-3.5.0-bin.tar.gz
ENV PATH $PATH:/usr/local/apache-maven/apache-maven-3.5.0/bin
RUN rm apache-maven-3.5.0-bin.tar.gz apache-maven-3.5.0-bin.tar.gz.md5
...
ADD http://archive.apache.org/dist/hbase/hbase-1.1.0/hbase-1.1.0-bin.tar.gz ./
RUN tar -xf hbase-1.1.0-bin.tar.gz
RUN rm hbase-1.1.0-bin.tar.gz
RUN ln -s hbase-1.1.0 hbase
RUN cp ../conf/hbase/hbase-site.xml hbase-1.1.0/conf/
RUN chmod +x hbase-1.1.0/bin/start-hbase.sh
